...
```
Your PC (client)  ──────────────────────────►  jsonplaceholder.typicode.com
                   sends HTTP GET / POST            (public REST API server)
                   requests via k6                          │
Your PC (client)  ◄──────────────────────────  returns JSON response
                   k6 measures response time,
                   throughput and error rate
```

- **My PC** is the **client** — Grafana k6 runs here and simulates many virtual users (VUs).
...
![Response time comparison](charts/response-time-comparison.png)
...
![Error rate](charts/error-rate.png)
